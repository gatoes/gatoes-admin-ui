  ...
  <div className="order-full-info">
    <ul style={{ listStyle: 'none', padding: 0 }}>
      <li style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
        ...
        <p style={{ margin: 0, whiteSpace: 'nowrap' }}>{restaurantData.ownerName}</p>
      </li>
      <li style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
        <label style={{ minWidth: '150px' }}>Activation Date</label>
        <p style={{ margin: 0, whiteSpace: 'nowrap' }}>{restaurantData.activationDate}</p>
      </li>
      <li style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
        <label style={{ minWidth: '150px' }}>Status</label>
        <p style={{ margin: 0, whiteSpace: 'nowrap' }}>{restaurantData.status}</p>
      </li>
    </ul>
  </div>
</div>
