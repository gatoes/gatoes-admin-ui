...
import { addBusinessZone, updateBusinessZoneSuccess } from '../../actions/regions';

class EditBusinessZone extends Component {
  constructor(props){
    super(props);
    this.state = {
  ...
  submitMenuForm(values){
    ...
    .then((result) => {
      this.props.updateBusinessZoneSuccess(result.data.data, itemIndex);
      toast.success('Business zone updated Successfully.');
      this.props.reset();
      this.props.updateBusinessZoneSlide(result.data.data);
    }).catch(error => {
...
EditBusinessZone = reduxForm({
  form: 'EditBusinessZoneValue',
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateBusinessZoneSuccess: (payload, index) => {
      dispatch(updateBusinessZoneSuccess(payload, index));
    },
  };
}

export default connect(mapStatesToProps, mapDispatchToProps)(EditBusinessZone);