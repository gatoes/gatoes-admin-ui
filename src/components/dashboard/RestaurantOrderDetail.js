...
import {getDetailRestaurentAnalytics} from '../../actions/orders';
...
class LocationOrderDetail extends Component {
	...
	downloadResult(){
		getDetailRestaurentAnalytics({'is_csv': true, 'startDate': this.state.startDate, 'endDate': this.state.endDate}).then((response) => {	
			this.setState({
	...
	fetchRecords(page_number=1){
	    getDetailRestaurentAnalytics({'pageNumber': page_number, 'startDate': this.state.startDate, 'endDate': this.state.endDate}).then((response) => {
	      	this.setState({
	      		shopRevenues: response.data.data
	      	});
	    });
	}
	...
	handlePageChange(pageNumber) {
  	...
  	render() {
  		const {shopRevenues, activePage, startDate, endDate, dateRangeString, startDownload } = this.state;
  		const srno = (activePage-1) * shopRevenues.limit;

  		//console.log('shopRevenues', shopRevenues);
		               		...
		               		<div className="col-sm-12">
		                  		<div className="result-listing">
		                     		<table>
		                        		...
		                        		<tbody>
			                           		{
			                           			shopRevenues && shopRevenues.revenue && shopRevenues.revenue.length>0 && shopRevenues.revenue.map((obj, index) => (
			                           				<tr>
		                     		...
		                     		<div className="pagination">
				                        <Pagination
								          	...
								          	itemsCountPerPage={shopRevenues.limit}
								          	totalItemsCount={shopRevenues.count ? shopRevenues.count : 0}
