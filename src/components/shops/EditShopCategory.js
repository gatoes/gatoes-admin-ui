...
import { addNewShopCateogry, shopCategoryDetailById } from '../../actions/shops';

class EditShopCategory extends Component {
  constructor(props) {
    super(props);
    this.state = {
      itemImageUrl: null
  ...
  componentWillMount(){
    shopCategoryDetailById({id : this.props.match.params.index}).then((response) => {
      this.props.initialize(response.data.data);
      this.setState({
        itemImageUrl: response.data.data.imageToShow && response.data.data.imageToShow.thumbnail == null ? null : response.data.data.imageToShow.thumbnail
      });
  ...
  submitMenuForm(values){
    ...
    return addNewShopCateogry(values)
    .then((result) => {
      toast.success('Category updated Successfully.');
      this.props.reset();
      this.props.history.push('/dashboard/manageshopcategory');
    }).catch(error => {
      ...
      <div className="container ani-ui-block shop-manager">
        <div className="row menu-top-block">
          <div className="col-sm-12 tl-block align-self-center">
            <h4 className="heading">Edit Restaurant Category</h4>
          </div>
...
EditShopCategory = reduxForm({
  form: 'EditShopCategoryValue',
